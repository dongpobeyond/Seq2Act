...
class OutputLayer(object):
  ...
  def __init__(self, vocab, hidden_size):
    ...
    self.nh = hidden_size
    self.nw = vocab.size()
    self.ew = vocab.all_size() - vocab.size()
    self.create_vars()
  ...
  def create_vars(self):
    ...
    self.w_out_contant = theano.shared(
        name='w_out_contant',
        value=numpy.zeros((self.ew, self.nh)).astype(int)
    )
  ...
  def write(self, h_t, attn_scores=None):
    ...
    """
    if attn_scores:
      scores = T.dot(h_t, T.concatenate([self.w_out, self.w_out_contant], axis=0).T)
      return T.nnet.softmax(T.concatenate([scores, attn_scores]))[0]
    else:
      return T.nnet.softmax(T.dot(h_t, T.concatenate([self.w_out, self.w_out_contant], axis=0).T))[0]
