...
OUT_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))),
    "seq2action_copy\\action\\geo880\\seq1")
...
def action2seq(action_seq, geo_controller, general_controller):
    ...
    for action_token in action_seq:
        gen_flag, gen_pre_action_class_out, gen_pre_arg_list_out, gen_pre_action_out, fun_trace_list_out = \
            general_controller.is_legal_action_then_read(gen_pre_action_class_in, gen_pre_arg_list_in, action_token, gen_pre_action_in, node_dict,
                                                         type_node_dict, entity_node_dict, operation_dict, edge_dict, return_node, db_triple,
                                                         fun_trace_list_in, for_controller=False)

        geo_flag = geo_controller.is_legal_action(gen_pre_action_class_out, [], action_token, '', node_dict, type_node_dict, entity_node_dict,
                                                       operation_dict, edge_dict, return_node, db_triple, [], for_controller=False)
...
def process(filename, base_file_name, geo_controller, general_controller):
    ...
    for (utterance, action_seq) in in_data:
        ...
        #print('action_seq: ' + action_seq)
        y = action2seq(action_seq, geo_controller, general_controller)
...
def test(geo_controller, general_controller):
    action_seq = 'add_operation:-:_sum add_node:-:C add_type_node:-:_state arg_node:-:C add_type_node:-:_area arg_node:-:C end_operation:-:_sum ope_for:-:C ope_in:-:_area ope_return:-:A return:-:A'
    print('actions: ' + action_seq)
    #print('len = ' + str(len(action_seq.split(' '))))
    lf = action2seq(action_seq, geo_controller, general_controller)
...
if __name__ == "__main__":
    geo_grammar_file = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))),
        "seq2action_copy\\ontology\\geo.grammar")
    general_grammar_file = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))),
        "seq2action_copy\\ontology\\general.grammar")
